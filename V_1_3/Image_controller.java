...
public class Image_controller {
    ...
    private ArrayList<int[]> Pixel_Non_list;  // 5size  [x,y, G strength, Gx, Gy]
    private Bitmap mbitmap;
    private int Num_Strong = 100;
    private int Num_Weak= 50;
    ...
    //6. Set the Pixel_Strong_list {Pixel_Non_list need & Num_Strong & Num_Weak need}
    public void Connecting_algorithm() {
        Log.d(TAG,"Connecting Start");
        int size = Pixel_Non_list.size();
        ArrayList<Integer> weak_num = new ArrayList<>();
        //We are going to analyze only the weak pixel
        for(int i =0; i<size; i++)
            //this is Threshold method
            if(Pixel_Non_list.get(i)[2] <Num_Strong && Pixel_Non_list.get(i)[2] > Num_Weak)
                weak_num.add(i);
        ArrayList<Integer> Check_list = new ArrayList<>();        //We are going to put the inspected pixel in this array
        int num = 0;//Current index from Pixel_Non_list. It is start from the 0
        while (weak_num.size()>0) {
            //if it is not included in Check list, we gonna start the analyzing from this pixel
            if (!Check_list.contains(num)) {
                    int last_x = Pixel_Non_list.get(size - 1)[0];
                    int last_y = Pixel_Non_list.get(size - 1)[1];
                    int w = last_x + 1;
                    int h = last_y + 1;
                    ...
                    for (int k = 0; k < Close_list.size(); k++) {
                        Pixel_Non_list.get(Close_list.get(k))[2] = color; //if it is connected with strong pixel it is 255 if not it is 0
                        Check_list.add(Close_list.get(k)); //Checking the number how many pixel we inspecting
                    }
                    Open_list.clear();
                    Close_list.clear();
                    num = 0;
    ...
    private boolean connecting(ArrayList<Integer> Close_list, ArrayList<Integer> Open_list, int w, int h, int num){
        ...
        if(x >0 && x<(w-1) && y >0 && y < (h-1)) {
            ...
            //if it is a weak pixel
                ...
                if (Pixel_Non_list.get(pixel_00)[2] > Num_Weak && !Close_list.contains(pixel_00)) {
                    Open_list.add(pixel_00);
                }
                if (Pixel_Non_list.get(pixel_01)[2] > Num_Weak && !Close_list.contains(pixel_01)) {
                    Open_list.add(pixel_01);
                }
                if (Pixel_Non_list.get(pixel_02)[2] > Num_Weak && !Close_list.contains(pixel_02)) {
                    Open_list.add(pixel_02);
                }
                if (Pixel_Non_list.get(pixel_10)[2] > Num_Weak && !Close_list.contains(pixel_10)) {
                    Open_list.add(pixel_10);
                }
                if (Pixel_Non_list.get(pixel_12)[2] > Num_Weak && !Close_list.contains(pixel_12)) {
                    Open_list.add(pixel_12);
                }
                if (Pixel_Non_list.get(pixel_20)[2] > Num_Weak && !Close_list.contains(pixel_20)) {
                    Open_list.add(pixel_20);
                }
                if (Pixel_Non_list.get(pixel_21)[2] > Num_Weak && !Close_list.contains(pixel_21)) {
                    Open_list.add(pixel_21);
                }
                if (Pixel_Non_list.get(pixel_22)[2] > Num_Weak && !Close_list.contains(pixel_22)) {
    ...
    if(mbitmap != null){
        ...
        startTime = System.currentTimeMillis();
        Connecting_algorithm();
        endTime = System.currentTimeMillis();
        duration= (endTime - startTime);
        txt.ShowingTxt("Set Strong Det : " + duration + "ms");
    ...
    private Bitmap drawing_bitmap(ArrayList<int[]> list) {
        ...
        if (Pixel_gray_list.equals(list) || Pixel_mask_list.equals(list) || Pixel_Non_list.equals(list)) {
            for (int i = 0; i < list.size(); i++) {
                ...
                if(color <Num_Strong && color > Num_Weak)
                    new_bitmap.setPixel(x, y, Color.rgb(0, 255, 0));
                else if(color < Num_Weak)
    ...
    //
    public int[] getPixel_Mask_list(){
        ArrayList<int[]> Data = Pixel_Non_list;
        int[] data = new int[Data.size()*5];
        int k=0;
        for(int i=0; i<Data.size(); i++) {
            data[k] = Data.get(i)[0];
            data[k+1] = Data.get(i)[1];
            data[k+2] = Data.get(i)[2];
            data[k+3] = Data.get(i)[3];
            data[k+4] = Data.get(i)[4];
            k+=5;
        }
        return data;
    }
